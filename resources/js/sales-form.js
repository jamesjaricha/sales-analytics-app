...
// Open modal to add quantity
window.openAddQuantityModal = function (row) {
    const quantityInput = row.querySelector(".quantity-input");
    ...
    const addQty = parseInt(amountToAdd);
...
function showToast(message, type = "info") {
    ...
    toast.className =
        "toast-notification shadow-lg rounded-lg px-4 py-3 text-white text-sm";
...
function checkForFlashMessages() {
    ...
    if (successMessage) {
        ...
        try {
            sessionStorage.removeItem(CONFIG.AUTOSAVE_KEY);
        } catch (e) {}
        // Only reset form if not a draft save
        if (
            !successMessage.dataset.flashSuccess.toLowerCase().includes("draft")
        ) {
            resetSalesForm();
...
function saveDraft() {
    if (
        !confirm(
            "Save this sales report as a draft? You can complete it later."
        )
    )
        return;
    try {
...
function displaySearchResults(products, input, query) {
    ...
    if (products.length === 0) {
        resultsDiv.innerHTML =
            '<div class="autocomplete-item text-gray-500">No products found</div>';
    } else {
        products.forEach(function (product) {
            ...
            item.innerHTML = `
                ...
                <div class="text-green-600 font-semibold mt-1">ZMW ${parseFloat(
                    product.price
                ).toFixed(2)}</div>
            `;
...
document.addEventListener("click", function (e) {
    if (
        !e.target.classList.contains("product-search-input") &&
...
function addDeduction() {
    ...
    container.appendChild(row);
    row.querySelector(".deduction-amount").addEventListener(
        "input",
        calculateTotals
    );
    row.querySelector(".remove-deduction-btn").addEventListener(
        "click",
        function () {
...
function calculateTotals() {
    ...
    items.forEach(function (item) {
        ...
        const price =
            parseFloat(item.querySelector(".price-input")?.value) || 0;
    ...
    const deductions = document.querySelectorAll(
        '#deductionsContainer input[name*="[amount]"]'
    );
    ...
    const cashAtHandEl = document.getElementById("cashAtHand");
    if (cashAtHandEl)
        cashAtHandEl.textContent = (totalSales - totalDeductions).toFixed(2);
}
...
function collectFormData() {
    const data = {
        sale_date:
            document.querySelector('input[name="sale_date"]')?.value || "",
        items: [],
        deductions: [],
    };
    document.querySelectorAll("#itemsContainer tr").forEach((tr) => {
        data.items.push({
            product_id: tr.querySelector(".product-id-field")?.value || "",
            product_name:
                tr.querySelector(".product-name-field")?.value ||
                tr.querySelector(".product-search-input")?.value ||
                "",
            quantity: tr.querySelector(".quantity-input")?.value || "",
            unit_price: tr.querySelector(".price-input")?.value || "",
        });
    });
...
function restoreFromAutosave() {
    try {
        ...
        const dateInput = document.querySelector('input[name="sale_date"]');
        if (saved.sale_date && dateInput) dateInput.value = saved.sale_date;
        const itemsContainer = document.getElementById("itemsContainer");
        if (itemsContainer) {
            itemsContainer.innerHTML = "";
            itemIndex = 0;
        ...
        if (dedContainer) {
            dedContainer.innerHTML = "";
            deductionIndex = 0;
            (saved.deductions || []).forEach((d) => {
                addDeduction();
                const row = dedContainer.lastElementChild;
                row.querySelector('input[name*="[description]"]').value =
                    d.description || "";
...
async function fetchAndRestoreDraftByDate(dateStr) {
    ...
    try {
        const resp = await fetch(
            `${routes.getDraft}?date=${encodeURIComponent(dateStr)}`
        );
        ...
        sessionStorage.setItem(
            CONFIG.AUTOSAVE_KEY,
            JSON.stringify({
                sale_date: form_data.sale_date || dateStr,
                items: form_data.items || [],
                deductions: form_data.deductions || [],
            })
        );
...
function autosaveNow() {
    try {
        ...
        sessionStorage.setItem(CONFIG.AUTOSAVE_KEY, JSON.stringify(data));
    } catch (e) {
...
function setupAutosave() {
    ...
    document
        ...
        ?.addEventListener("input", autosaveNow);
}
...
window.addEventListener("DOMContentLoaded", function () {
    ...
    document
        ...
        ?.addEventListener("submit", submitQuickCreate);
    ...
    if (formMode === "edit") {
        ...
        const dateInput = document.querySelector('input[name="sale_date"]');
        if (dateInput) {
            fetchAndRestoreDraftByDate(dateInput.value).then((found) => {
                if (!found) addItem();
            });
        } else {
            addItem();
