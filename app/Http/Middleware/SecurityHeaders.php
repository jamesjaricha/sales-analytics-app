...
class SecurityHeaders
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (app()->environment('production')) {
            ...
            $response->headers->set('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' blob: data: {$viteUrl} {$kaspersky}; style-src 'self' 'unsafe-inline' blob: data: {$viteUrl} {$kaspersky}; img-src 'self' data: blob:; font-src 'self' data:; connect-src 'self' ws: wss: {$viteWs} {$viteUrl} {$kaspersky}; object-src 'none'; base-uri 'self';");
        }
